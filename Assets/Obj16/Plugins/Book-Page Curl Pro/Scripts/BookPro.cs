...
public class BookPro : MonoBehaviour
{
    ...
    public delegate void bookflip_cases(string cases);
    public static event bookflip_cases onBookFlipCasecall;
    /// <summary>
    ...
    {
        FindObjectOfType<Obj16_SceneManager>().temp = this.gameObject;
        if (interactable && !tweening)
        {
            FindObjectOfType<Obj16_SceneManager>().stop_timer();
            DragRightPageToPoint(transformPointMousePosition(Input.mousePosition));
            if (gameObject.name == "fold3_book2" || gameObject.name == "fold3_book3")
            {
    ...
    public void ReleasePage()
    {
        if (pageDragging)
        {
            ...
            if (FindObjectOfType<Obj16_SceneManager>().fold1)
            {
                if ((distanceToLeft) > 10.0f && mode == FlipMode.RightToLeft)
                    TweenBack();
                else if (distanceToRight > 10.0f && mode == FlipMode.LeftToRight)
                    TweenBack();
                else
                    TweenForward();
            }
            else if (FindObjectOfType<Obj16_SceneManager>().fold2)
            {
                if ((distanceToLeft) > 40.0f && mode == FlipMode.RightToLeft)
                    TweenBack();
                else if (distanceToRight > 40.0f && mode == FlipMode.LeftToRight)
                    TweenBack();
                else
                    TweenForward();
            }
    ...
    void Update()
    {
        ...
        dottedLine = GameObject.FindGameObjectWithTag("dotted line").GetComponent<Image>();
        checkMeeting();
    }
    ...
    public void TweenForward()
    {
        if (mode == FlipMode.RightToLeft)
        {
            ...
            if (FindObjectOfType<Obj16_SceneManager>().fold3_count == 1)
            {
                if (this.gameObject.tag == "book")
                {
                    ...
                    if (gameObject.name == "fold3_book2" || gameObject.name == "fold3_book3")
                    {
                        FindObjectOfType<Obj16_SceneManager>().fold3_top.GetComponent<Image>().color = new Color(0.6039216f, 0.9372549f, 0.9607843f);
                        FindObjectOfType<Obj16_SceneManager>().fold3_bottom.GetComponent<Image>().color = new Color(0.6039216f, 0.9372549f, 0.9607843f);
                    }
                    this.gameObject.SetActive(false);
                    //GameObject.Find("RightHotSpot").GetComponent<Image>().enabled = true;

                    FindObjectOfType<Obj16_SceneManager>().fold3_count++;
                    FindObjectOfType<Obj16_SceneManager>().start_timer();
                }
            }
            else if (onBookFlipCasecall != null)
            {
                onBookFlipCasecall("right_manner");
            }
        ...
        else
        {
            ...
            if (onBookFlipCasecall != null)
            {
                onBookFlipCasecall("wrong_end");
            }
        }
    ...
    public void TweenBack()
    {
        ...
        if (mode == FlipMode.RightToLeft)
        {
            ...
            if (onBookFlipCasecall != null)
            {
                onBookFlipCasecall("wrong_manner");
            }
        }
        else
        {
